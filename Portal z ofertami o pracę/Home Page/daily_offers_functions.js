...
const dailyApplyButtons = document.querySelectorAll(
  '.day-offer-card > div > button'
);
...
function createDailyApplySection() {
  if (dailySectionHTML.children.length != 0)
    ...
    dailySectionHTML.innerHTML = browserDefaultLanguage.includes('pl')
      ? returnApplyHTML(dailyOfferObjectPL)
      : returnApplyHTML(dailyOfferObjectEN);
